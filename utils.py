...
def show_epoch_info(phase, log_path, info):
    ...
    print_log(log_path, 'Group Activity Accuracy: %.2f%%, acc: %.2f%%, Loss: %.5f, Using %.1f seconds'%(
                info['activities_acc'], info['acc'], info['loss'], info['time']))
    ...
    if 'actions_acc' in info.keys():
        print_log(log_path, '\nActions acc:{:.2f}%'.format(info['actions_acc']))
    print_log(log_path, '\n')
